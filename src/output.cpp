...
public:
    LameTone(mix_modes mixmode, int msec, unsigned int hz = 0) : _data(NULL), _bytes(0) {
        ...
        if (lame) {
            _bytes = lame_encode_buffer_ieee_float(lame, buf, (mixmode == MM_STEREO ? buf : NULL), samples, _data,
                                                   LAMEBUF_SIZE);
            if (_bytes > 0) {
...
static bool output_file_ready(channel_t *channel, file_data *fdata, mix_modes mixmode, int is_audio) {
    ...
    size_t file_path_len = strlen(fdata->basename) + strlen(timestamp) + strlen(fdata->suffix) +
                           11; // include space for '\0' and possible freq in Hz
    fdata->file_path = (char *) XCALLOC(1, file_path_len);
    if (fdata->include_freq) {
        sprintf(fdata->file_path, "%s%s_%d%s", fdata->basename, timestamp,
                channel->freqlist[channel->freq_idx].frequency, fdata->suffix);
    } else {
        ...
        if (channel->outputs[k].type == O_ICECAST) {
            ...
            if (ret != SHOUTERR_SUCCESS || shout_queuelen(icecast->shout) > MAX_SHOUT_QUEUELEN) {
                ...
                if (channel->freqlist[channel->freq_idx].label != NULL) {
                    if (shout_metadata_add(meta, "song", channel->freqlist[channel->freq_idx].label) !=
                        SHOUTERR_SUCCESS) {
                        log(LOG_WARNING, "Failed to add shout metadata\n");
                    }
                } else {
                    snprintf(description, sizeof(description), "%.3f MHz",
                             channel->freqlist[channel->freq_idx].frequency / 1000000.0);
            ...
            mixer_put_samples(mdata->mixer, mdata->input, channel->waveout, channel->axcindicate != NO_SIGNAL,
                              WAVE_BATCH);
            ...
            pulse_write_stream(pdata, channel->mode, channel->waveout, channel->waveout_r,
                               (size_t) WAVE_BATCH * sizeof(float));
#endif
        }
    }
}
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        for (int j = 0; j < dev->channel_count; j++) {
            ...
            for (int k = 0; k < channel->freq_count; k++) {
                print_channel_metric(f, "channel_noise_level", channel->freqlist[k].frequency,
                                     channel->freqlist[k].label);
...
static void output_channel_dbfs_noise_levels(FILE *f) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        for (int j = 0; j < dev->channel_count; j++) {
            channel_t *channel = devices[i].channels + j;
            for (int k = 0; k < channel->freq_count; k++) {
                print_channel_metric(f, "channel_dbfs_noise_level", channel->freqlist[k].frequency,
                                     channel->freqlist[k].label);
...
static void output_channel_signal_levels(FILE *f) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        for (int j = 0; j < dev->channel_count; j++) {
            channel_t *channel = devices[i].channels + j;
            for (int k = 0; k < channel->freq_count; k++) {
                print_channel_metric(f, "channel_signal_level", channel->freqlist[k].frequency,
                                     channel->freqlist[k].label);
...
static void output_channel_dbfs_signal_levels(FILE *f) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        for (int j = 0; j < dev->channel_count; j++) {
            channel_t *channel = devices[i].channels + j;
            for (int k = 0; k < channel->freq_count; k++) {
                print_channel_metric(f, "channel_dbfs_signal_level", channel->freqlist[k].frequency,
                                     channel->freqlist[k].label);
...
static void output_channel_squelch_levels(FILE *f) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        for (int j = 0; j < dev->channel_count; j++) {
            channel_t *channel = devices[i].channels + j;
            for (int k = 0; k < channel->freq_count; k++) {
                print_channel_metric(f, "channel_squelch_level", channel->freqlist[k].frequency,
                                     channel->freqlist[k].label);
...
static void output_channel_squelch_counter(FILE *f) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        for (int j = 0; j < dev->channel_count; j++) {
            channel_t *channel = devices[i].channels + j;
            for (int k = 0; k < channel->freq_count; k++) {
                print_channel_metric(f, "channel_squelch_counter", channel->freqlist[k].frequency,
                                     channel->freqlist[k].label);
...
static void output_channel_flappy_counter(FILE *f) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        for (int j = 0; j < dev->channel_count; j++) {
            channel_t *channel = devices[i].channels + j;
            for (int k = 0; k < channel->freq_count; k++) {
                print_channel_metric(f, "channel_flappy_counter", channel->freqlist[k].frequency,
                                     channel->freqlist[k].label);
...
static void output_channel_ctcss_counter(FILE *f) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        for (int j = 0; j < dev->channel_count; j++) {
            channel_t *channel = devices[i].channels + j;
            for (int k = 0; k < channel->freq_count; k++) {
                print_channel_metric(f, "channel_ctcss_counter", channel->freqlist[k].frequency,
                                     channel->freqlist[k].label);
...
static void output_channel_no_ctcss_counter(FILE *f) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        for (int j = 0; j < dev->channel_count; j++) {
            channel_t *channel = devices[i].channels + j;
            for (int k = 0; k < channel->freq_count; k++) {
                print_channel_metric(f, "channel_no_ctcss_counter", channel->freqlist[k].frequency,
                                     channel->freqlist[k].label);
...
static void output_channel_activity_counters(FILE *f) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        for (int j = 0; j < dev->channel_count; j++) {
            channel_t *channel = devices[i].channels + j;
            for (int k = 0; k < channel->freq_count; k++) {
                print_channel_metric(f, "channel_activity_counter", channel->freqlist[k].frequency,
                                     channel->freqlist[k].label);
...
void *output_thread(void *param) {
    ...
    debug_print("Starting output thread, devices %d:%d, mixers %d:%d, signal %p\n", output_param->device_start,
                output_param->device_end, output_param->mixer_start, output_param->mixer_end, output_param->mp3_signal);
    ...
    while (!do_exit) {
        ...
        debug_bulk_print("mixeroutput: %lu.%lu %lu\n", te.tv_sec, (unsigned long) te.tv_usec,
                         (te.tv_sec - ts.tv_sec) * 1000000UL + te.tv_usec - ts.tv_usec);
        ts.tv_sec = te.tv_sec;
